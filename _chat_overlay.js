(function() {
  ...
  function renderMarkdown(text) {
    ...
    // 待办事项（Obsidian 风格）
    s = s.replace(/^(\s*)-\s+\[([ xX])\]\s+(.+)$/gm, function(_, indent, done, text) {
      var checked = done === 'x' || done === 'X';
      var checkbox = '<span class="hdc-todo-checkbox" data-done="' + (checked ? '1' : '0') + '" style="cursor:pointer;display:inline-block;width:14px;height:14px;border:1px solid var(--hdc-border);border-radius:3px;margin-right:6px;vertical-align:middle;text-align:center;line-height:12px;font-size:10px;' + (checked ? 'background:var(--hdc-accent);color:#fff' : 'background:transparent') + '">' + (checked ? '✓' : '') + '</span>';
      var style = checked ? 'text-decoration:line-through;opacity:0.6' : '';
      // 保留原始缩进，复选框紧跟在 [ ] 的位置
      return indent + checkbox + '<span style="' + style + '">' + text + '</span>';
    });
